...
class RPCSymbolProcessor(
    ...
    private fun processService(serviceDeclaration: KSClassDeclaration, file: KSFile): RPCServiceDeclaration {
        ...
        if (serviceDeclaration.modifiers.intersect(DENY_LIST_SERVICE_MODIFIERS).isNotEmpty()) {
            codegenError("RPC Service can not have any of these modifiers: ${DENY_LIST_SERVICE_MODIFIERS.joinToString()}", serviceDeclaration)
        }
        ...
        return RPCServiceDeclaration(
            ...
            fullName = serviceDeclaration.qualifiedName?.asString() ?: codegenError("Expected service qualified name", serviceDeclaration),
    ...
    private fun processServiceFunction(functionDeclaration: KSFunctionDeclaration): RPCServiceDeclaration.Function {
        ...
        if (functionDeclaration.modifiers.intersect(DENY_LIST_FUNCTION_MODIFIERS).isNotEmpty()) {
            codegenError("RPC Service function can not have any of these modifiers: ${DENY_LIST_FUNCTION_MODIFIERS.joinToString()}", functionDeclaration)
        }
    ...
    private fun processFunctionArgument(functionDeclaration: KSFunctionDeclaration, argument: KSValueParameter): RPCServiceDeclaration.Function.Argument {
        return RPCServiceDeclaration.Function.Argument(
            name = argument.name?.getShortName() ?: codegenError("Expected function argument name", functionDeclaration),
    ...
    companion object {
        private val DENY_LIST_SERVICE_MODIFIERS = setOf(Modifier.PRIVATE, Modifier.SEALED, Modifier.INTERNAL, Modifier.EXPECT, Modifier.ACTUAL, Modifier.EXTERNAL)

        private val DENY_LIST_FUNCTION_MODIFIERS = setOf(Modifier.PRIVATE, Modifier.INTERNAL, Modifier.INFIX, Modifier.OPERATOR, Modifier.EXPECT, Modifier.ACTUAL, Modifier.FINAL, Modifier.TAILREC, Modifier.OVERRIDE)
