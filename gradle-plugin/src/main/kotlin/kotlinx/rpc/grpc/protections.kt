    ...
    afterEvaluate {
        ...
        if (isBufPluginApplied && !isProtobufPluginApplied) {
            throw GradleException(
                "Buf plugin ($BUF_PLUGIN_ID) can't be applied to the project, it is not compatible with the Rpc Gradle Plugin "
            )
        }

        if (isProtobufPluginApplied && !isBufPluginApplied) {
            throw GradleException(
                "Protobuf plugin ($PROTOBUF_PLUGIN_ID) can't be applied to the project, it is not compatible with the Rpc Gradle Plugin "
            )
        }

        if (isBufPluginApplied) {
            throw GradleException(
                "Both Buf ($BUF_PLUGIN_ID) and Protobuf ($PROTOBUF_PLUGIN_ID) plugins can't be applied to the project, they are not compatible with the Rpc Gradle Plugin "
            )
        }
    }
}
