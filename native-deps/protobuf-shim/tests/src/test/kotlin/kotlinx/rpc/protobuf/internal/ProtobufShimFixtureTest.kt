...
class ProtobufShimFixtureTest {
    private val resourcesDir = kotlin.io.path.Path("src", "test", "resources")
    private val verificationRepositoryDir = kotlin.io.path.Path(System.getProperty("protobufShimVerificationRepoDir"))
    private val protobufShimVersion = System.getProperty("protobufShimVersion")
    private val kotlinVersion = System.getProperty("protobufShimKotlinVersion")
    private val hostTargetDeclaration = System.getProperty("protobufShimHostTargetDeclaration")
    private val hostCompileTask = System.getProperty("protobufShimHostCompileTask")
    private val hostPublicationSuffix = System.getProperty("protobufShimHostPublicationSuffix")
    ...
    fun publishedCinteropKlibCarriesInternalAnnotationMarker() {
        val cinteropKlib = verificationRepositoryDir.resolve(
            "org/jetbrains/kotlinx/kotlinx-rpc-protobuf-shim-$hostPublicationSuffix/" +
                "$protobufShimVersion/" +
                "kotlinx-rpc-protobuf-shim-$hostPublicationSuffix-$protobufShimVersion-cinterop-libprotowire.klib",
        )
        ...
        ZipFile(cinteropKlib.toFile()).use { zip ->
            ...
            assertContains(manifest, "kotlinx-rpc-protobuf-shim-annotation")

            val knmEntry = zip.entries().asSequence().firstOrNull { entry ->
                !entry.isDirectory &&
                    entry.name.startsWith("default/linkdata/package_kotlinx.rpc.protobuf.internal.cinterop/") &&
                    entry.name.endsWith(".knm")
                }
            assertTrue(
                knmEntry != null,
                "Missing package_kotlinx.rpc.protobuf.internal.cinterop knm entry in ${cinteropKlib.pathString}",
            )
    ...
    private fun prepareScenarioProject(scenarioName: String): Path {
        ...
        projectDir.resolve("settings.gradle.kts").writeText(
            settingsTemplate.replace("<test-name>", "protobuf-shim-$scenarioName"),
        )
        projectDir.resolve("gradle.properties").writeText(propertiesTemplate)
        projectDir.resolve("build.gradle.kts").writeText(
            buildTemplate
                .replace("<kotlin-version>", kotlinVersion)
                .replace("<protobuf-shim-version>", protobufShimVersion)
