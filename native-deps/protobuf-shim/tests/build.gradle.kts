...
val publishHostCore = project(":kotlinx-rpc-protobuf-shim").tasks.named("publish${hostTarget.publicationTaskSuffix}PublicationToVerificationRepository")
val publishRootAnnotation = project(":kotlinx-rpc-protobuf-shim-annotation").tasks.named("publishKotlinMultiplatformPublicationToVerificationRepository")
val publishHostAnnotation = project(":kotlinx-rpc-protobuf-shim-annotation").tasks.named("publish${hostTarget.publicationTaskSuffix}PublicationToVerificationRepository")
...
fun Test.configureFixtureVerification() {
    ...
    systemProperty("protobufShimVersion", rootProject.version.toString())
...
fun registerTaggedTest(name: String, tag: String) = tasks.register<Test>(name) {
    group = "verification"
    description = "Runs protobuf-shim fixture tests tagged '$tag'."
