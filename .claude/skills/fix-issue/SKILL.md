...
3. Create draft PR with `gh pr create --draft`. Include `Fixes #NNN` in the body if
   ...
   Subsystem, Problem Description (link to YT issue), Solution. Clearly indicate this
   is a **fully autonomous autogenerated PR** (not just "created by an agent" — make it
   explicit that no human reviewed the code before submission).
...
Set state to `Fixed in Branch` via `youtrack-agent`.
