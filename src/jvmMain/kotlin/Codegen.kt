...
class MyServiceClient(private val engine: RPCEngine) : MyService {
    override suspend fun simpleRequest(): String {
        ...
        val result = engine.call(RPCCallInfo(Unit, emptyList(), returnType))
    ...
    override suspend fun simpleWithParams(name: String): String {
        val returnType = typeOf<String>()
        val result = engine.call(RPCCallInfo(SimpleWithParams_Data(name), listOf(typeOf<String>()), returnType))
        check(result is String)
        return result
    }

    override suspend fun streamRequest(messages: Flow<String>): Int {
        val result = engine.call(RPCCallInfo(StreamRequest_Data(messages), listOf(typeOf<Flow<String>>()), typeOf<Int>()))
        check(result is Int)
        return result
    }
