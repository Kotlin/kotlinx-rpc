...
import util.other.filterDirectory
import util.other.name
import java.io.File
...
fun Collection<File>.mostSpecificVersionOrLatest(kotlinVersion: KotlinVersion): File? {
    return mostSpecificByVersionOrNull(kotlinVersion)
        ?: singleOrNull { it.name == DirectoryNames.LATEST_SOURCE_DIR }
}
...
private class CompilerModuleVersion(fullName: String, prefix: String) : Comparable<CompilerModuleVersion> {
