...
private fun <Request, Response> CoroutineScope.serverMethodDefinition(
    ...
    _root_ide_package_.kotlinx.rpc.grpc.server.internal.serverMethodDefinition(
        descriptor,
        serverCallHandler(descriptor, responseKType, interceptors, implementation)
    )
...
private fun <Request, Response> CoroutineScope.serverCallHandler(
    ...
    interceptors: List<ServerInterceptor>,
    implementation: (Flow<Request>) -> Flow<Response>,
): kotlinx.rpc.grpc.server.internal.ServerCallHandler<Request, Response> =
    _root_ide_package_.kotlinx.rpc.grpc.server.internal.ServerCallHandler { call, headers ->
...
private fun <Request, Response> CoroutineScope.serverCallListenerImpl(
    descriptor: MethodDescriptor<Request, Response>,
    handler: kotlinx.rpc.grpc.server.internal.ServerCall<Request, Response>,
    responseKType: KType,
    interceptors: List<ServerInterceptor>,
    implementation: (Flow<Request>) -> Flow<Response>,
    requestHeaders: GrpcMetadata,
): kotlinx.rpc.grpc.server.internal.ServerCall.Listener<Request> {
    val ready = Ready { handler.isReady() }
    val requestsChannel = Channel<Request>(1)

    val requestsStarted = _root_ide_package_.kotlinx.rpc.grpc.server.internal.AtomicBoolean(false) // enforces read-once
    ...
    val rpcJob = launch() {
        val mutex = Mutex()
        val headersSent =
            _root_ide_package_.kotlinx.rpc.grpc.server.internal.AtomicBoolean(false) // enforces only sending headers once
        val failure = runCatching {
            serverCallScope.proceed(requests).collect { response ->
                @Suppress("UNCHECKED_CAST")
                // fix for KRPC-173
                val value = if (responseKType == _root_ide_package_.kotlinx.rpc.grpc.server.internal.unitKType) Unit as Response else response
    ...
    return _root_ide_package_.kotlinx.rpc.grpc.server.internal.serverCallListener(
        state = _root_ide_package_.kotlinx.rpc.grpc.server.internal.ServerCallListenerState(),
        onCancel = {
