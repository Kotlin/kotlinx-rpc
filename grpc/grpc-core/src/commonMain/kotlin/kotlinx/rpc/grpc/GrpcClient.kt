...
import kotlinx.rpc.grpc.descriptor.GrpcDelegate
...
public class GrpcClient(private val channel: ManagedChannel) : RpcClient {
    ...
    private val stubs = ConcurrentHashMap<Long, GrpcDelegate>()
    ...
    private fun RpcCall.delegate(): GrpcDelegate {
        val grpc = (descriptor as? GrpcServiceDescriptor<*>)
            ?: error("Service ${descriptor.fqName} is not a gRPC service")

        return stubs.computeIfAbsent(serviceId) { grpc.delegate(channel) }
    }
