...
`kotlinx.rpc` provides you with
runtime [dependencies](https://docs.gradle.org/current/userguide/declaring_dependencies.html)
...
[simple Kotlin/JVM projects](https://kotlinlang.org/docs/get-started-with-jvm-gradle-project.html#explore-the-build-script)
only.
...
see [the simple single-module Ktor App example](%repo-tree-path%/samples/simple-ktor-app)
and the [Ktor web app example](%repo-tree-path%/samples/ktor-web-app) respectively.

In the latter you will find usage
of [Gradle version catalogs](https://docs.gradle.org/current/userguide/platforms.html),
so the dependency declaration may look like this:

[//]: # (TODO change language to toml when Writersite supports it)
...
> Note that we use version `%kotlinx-rpc-version%` here, instead of `%kotlin-version%-%kotlinx-rpc-version%`
...
dependencies {
    // versions are set automatically to %kotlin-version%-%kotlinx-rpc-version% 
...
It has the same BOM functionality as `org.jetbrains.kotlinx.rpc.platform`, and it also sets up code generation
configurations.
...
Here `%kotlin-version%` is the version of Kotlin of your project.
...
So if the last stable Kotlin version is `%kotlin-version%`, as at the time of writing this guide,
...
(So, for core version `%kotlinx-rpc-version%`, there are `1.7.0-%kotlinx-rpc-version%`,
`1.7.0-%kotlinx-rpc-version%`, ... , `%kotlin-version%-%kotlinx-rpc-version%` versions present).
...
The `kotlinx.rpc` library provides integration with the [Ktor framework](https://ktor.io) with the in-house RPC
protocol.
...
For more details and complete examples, see the [code samples](%repo-tree-path%/samples).
